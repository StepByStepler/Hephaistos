...
class ChunkSectionReader constructor(val version: SupportedVersion, val nbt: NBTCompound) {
    ...
    fun getUncompressedBiomeIndices(): IntArray {
        ...
        if(biomePalette.size == 1) {
            return IntArray(ChunkSection.BiomeArraySize) { 0 }
        } else {
            ...
            return unpack(compressedBiomes, sizeInBits).sliceArray(0 until ChunkSection.BiomeArraySize)
        }
    }
    ...
    fun getBiomeInformation(): SectionBiomeInformation {
        ...
        if("data" !in biomesNBT) {
            ...
            biomes = Array<String>(ChunkSection.BiomeArraySize) { Biome.UnknownBiome }
